...
# ----------------------------------------------------
    ...
    def generuj_program_indywidualny(numery_zawodnikow):

        # Używamy wyłącznie numerów zawodników, którzy rzeczywiście
        # zostali wpisani. Każdy bieg ma dokładnie 4 różne osoby.
        numery = list(numery_zawodnikow)
        ...
        # Prosty rotacyjny system 20 biegów.
        # Dzięki temu działa zarówno dla 4, jak i dla 5-20 zawodników.
        for bieg in range(20):
            start = (bieg * 3) % liczba
            grupa = []

            for i in range(4):
                nr = numery[(start + i) % liczba]
                if nr not in grupa:
                    grupa.append(nr)

            # Dla bezpieczeństwa uzupełniamy grupę, gdyby liczba
            # zawodników była nietypowa.
            if len(grupa) < 4:
                for nr in numery:
                    if nr not in grupa:
                        grupa.append(nr)
    ...
    zawodnicy_aktualni = zawodnicy_ind()
    numery_aktualnych_zawodnikow = [
        x["nr"] for x in zawodnicy_aktualni
    ]
    liczba_zawodnikow = len(numery_aktualnych_zawodnikow)

    # Program jest generowany dopiero po wpisaniu zawodników.
    # Po rozpoczęciu turnieju pozostaje niezmieniony.
    if (
        "ind_program" not in st.session_state
        or (
            st.session_state.get("ind_current_heat", 0) == 0
            and st.session_state.get("ind_program_numery")
            != numery_aktualnych_zawodnikow
        )
    ):
        st.session_state.ind_program = generuj_program_indywidualny(
            numery_aktualnych_zawodnikow
        )
        st.session_state.ind_program_numery = list(
            numery_aktualnych_zawodnikow
        )
    ...
    with tab_ind_mecz:
        ...
        if len(zawodnicy) < 4:
            ...
            if st.button(
                ...
                st.session_state.ind_program = generuj_program_indywidualny(
                    [x["nr"] for x in zawodnicy]
                )
                st.session_state.ind_program_numery = [
                    x["nr"] for x in zawodnicy
                ]
